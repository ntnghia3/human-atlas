...
import {matchesTerminologyQuery,TERMINOLOGY_OVERLAY,resolveConceptName,type TerminologyOverlay} from './terminology.ts';
...
export function atlasTools(atlas:Atlas,inspect:(concept:Concept)=>void,overlay:TerminologyOverlay=TERMINOLOGY_OVERLAY):Tool[]{return [
 {name:'find_anatomy',description:'Find anatomical structures by name or source atlas identifier in this atlas.',inputSchema:{type:'object',properties:{query:{type:'string',minLength:1}},required:['query'],additionalProperties:false},annotations:{readOnlyHint:true},execute(input){const data=record(input);if(typeof data.query!=='string'||!data.query.trim())throw new Error('A nonempty query is required.');return atlas.concepts.filter(c=>matchesTerminologyQuery(c,data.query as string,overlay)).slice(0,30).map(c=>({id:c.id,name:resolveConceptName(c,'en',overlay),pieces:c.elements.length}));}},
