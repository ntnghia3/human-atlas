...
import {matchesTerminologyQuery,TERMINOLOGY_OVERLAY,TERMINOLOGY_SOURCES,resolveConceptName,type TerminologyOverlay,type TerminologySourceCatalog} from './terminology.ts';
...
export function atlasTools(atlas:Atlas,inspect:(concept:Concept)=>void,overlay:TerminologyOverlay=TERMINOLOGY_OVERLAY,sourceCatalog:TerminologySourceCatalog=TERMINOLOGY_SOURCES):Tool[]{return [
 {name:'find_anatomy',description:'Find anatomical structures by name or source atlas identifier in this atlas.',inputSchema:{type:'object',properties:{query:{type:'string',minLength:1}},required:['query'],additionalProperties:false},annotations:{readOnlyHint:true},execute(input){const data=record(input);if(typeof data.query!=='string'||!data.query.trim())throw new Error('A nonempty query is required.');return atlas.concepts.filter(c=>matchesTerminologyQuery(c,data.query as string,overlay,sourceCatalog)).slice(0,30).map(c=>({id:c.id,name:resolveConceptName(c,'en',overlay,sourceCatalog),pieces:c.elements.length}));}},
