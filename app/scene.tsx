...
export type SceneError = 'webgl'|'atlasLoad'|'chunkLoad'|'contextLost';
interface Props {atlas:Atlas;state:SceneState;ariaLabel:string;getPartLabel:(part:Part)=>string;onSelect:(id:string)=>void;onProgress:(n:number)=>void;onError:(error:SceneError)=>void}
export default function AnatomyScene({atlas,state,ariaLabel,getPartLabel,onSelect,onProgress,onError}:Props){
 const host=useRef<HTMLDivElement>(null),latest=useRef(state),select=useRef(onSelect),labelForPart=useRef(getPartLabel),reportError=useRef(onError);
 latest.current=state;select.current=onSelect;labelForPart.current=getPartLabel;reportError.current=onError;
 useEffect(()=>{
  const el=host.current!;let disposed=false,frame=0,dirty=true,ready=false,lastView='',lastReset=-1,lastIsolate='',layoutKey='',amount=0;
  let lastState:SceneState|null=null;
  const abort=new AbortController();
  let renderer:T.WebGLRenderer;
  try{renderer=new T.WebGLRenderer({antialias:true,alpha:false,powerPreference:'high-performance'});}catch{reportError.current('webgl');return;}
  ...
  (async()=>{try{let cursor=0;await Promise.all(Array.from({length:3},async()=>{while(cursor<atlas.chunks.length){const i=cursor++;await loadChunk(i);}}));if(!disposed){ready=true;dirty=true;}}catch{if(!disposed)reportError.current('chunkLoad');}})();
  ...
  const contextLost=(e:Event)=>{e.preventDefault();reportError.current('contextLost');};renderer.domElement.addEventListener('webglcontextlost',contextLost);
 ...
 useEffect(()=>{const canvas=host.current?.querySelector('canvas');if(canvas)canvas.setAttribute('aria-label',ariaLabel);},[ariaLabel]);
